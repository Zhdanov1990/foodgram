...
class RecipeReadSerializer(serializers.ModelSerializer):
    ...
    def get_is_favorited(self, obj):
        user = self.context['request'].user
        return user.is_authenticated and obj.in_favorites.filter(pk=user.pk).exists()

    def get_is_in_shopping_cart(self, obj):
        user = self.context['request'].user
        return user.is_authenticated and obj.in_shopping_carts.filter(pk=user.pk).exists()
